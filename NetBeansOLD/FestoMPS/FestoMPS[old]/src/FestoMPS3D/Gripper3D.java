...
public class Gripper3D extends ConveyorUnit3D {
    ...
    protected BranchGroup createStructure() {
        ...
        Box columnA = new Box( 0.5f, 8, 0.5f, null );
        ...
        Box columnB = new Box( 0.5f, 8, 0.5f, null );
        ...
        Box columnC = new Box( 0.5f, 8, 0.5f, null );
        ...
        Box columnD = new Box( 0.5f, 8, 0.5f, null );
        ...
        Box barA = new Box( 0.5f, 0.5f, 9, null );
        ...
        Box barB = new Box( 0.5f, 0.5f, 9, null );
        ...
        Box barC = new Box( 4, 0.5f, 0.5f, null );
        ...
        Cylinder motorZ = new Cylinder( 0.5f, 1, AppearanceAttributes.getColorApp( "green" ) );
        ...
        Cylinder motorX = new Cylinder( 0.5f, 1, AppearanceAttributes.getColorApp( "green" ) );
...
//        workpieceSensorTGT.addChild( diffuseSensor );
    ...
    public void setAnimation( String move ) {
        ...
        switch( move ) {
            case "grip.levelDown":
                if( i == 0 ) {
                    System.out.println("gU3D switch 1");
                    units[0].setAnimation( "grip.levelDown" );
                    i = 1;
                } else {
                    System.out.println("gU3D switch 2");
                    units[0].setAnimation( "grip.levelUp" );
                    i = 1;
                }
                break;
            default:
                System.out.println("gU3D switch problem");
                break;
        }
...
class ArmMotion extends Behavior {
    
    protected TransformGroup targetTG = null;
    protected PositionInterpolator armFront = null;
    protected PositionInterpolator armBack = null;
    protected Transform3D armFrontTF3D = null;
    protected Transform3D armBackTF3D = null;
    
    public ArmMotion( TransformGroup targetTG ) {
        
        this.targetTG = targetTG;
        
        armFrontTF3D = new Transform3D();
        armFrontTF3D.set( new Vector3f(0, 0.0f, 2.0f) );
        armBackTF3D = new Transform3D();
        armBackTF3D.set( new Vector3f(0, 0.0f, -2.0f) );
        
        armFront = new PositionInterpolator( new Alpha(1,3000), targetTG, armFrontTF3D, 0.0f, 2.0f );
        armBack = new PositionInterpolator( new Alpha(1,3000), targetTG, armBackTF3D, 0.0f, 2.0f );
    }
